...
class CombinedLoss(nn.Module):
    ...
    Optionally adds a release-head pinball term when the model predicts
    reservoir releases alongside inflows.
    """
    def __init__(self, alpha: float = 1.0, quantiles: List[float] = [0.1, 0.5, 0.9], threshold: float = 0.5, scaling_factor: float = 2.0, release_weight: float = 0.5):
        """
        ...
        Args:
            ...
            release_weight (float): Weight for the optional release-head pinball term.
        """
        super().__init__()
        self.alpha = alpha
        self.release_weight = release_weight
        self.pinball = PinballLoss(quantiles=quantiles)
        self.release_pinball = PinballLoss(quantiles=quantiles)
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor, oni: torch.Tensor,
                y_pred_rel: torch.Tensor | None = None, y_true_rel: torch.Tensor | None = None) -> torch.Tensor:
        """
        ...
        Args:
            ...
            y_pred_rel/y_true_rel: Optional release-head predictions/targets (same shapes).
        ...
        total = self.alpha * l_pinball + l_asym
        if y_pred_rel is not None and y_true_rel is not None:
            total = total + self.release_weight * self.release_pinball(y_pred_rel, y_true_rel)
        return total
