...
class ReservoirGNN(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        # Optional release head: predicts weekly reservoir releases (MCM) with
        # the same fused representation — enables LEVEL forecasts via mass balance.
        self.predict_releases = bool(config.get("predict_releases", False))
        if self.predict_releases:
            self.release_head = QuantilePredictionHead(
                input_dim=fused_dim,
                num_weeks=config.get("num_weeks", 12),
                num_quantiles=config.get("num_quantiles", 3),
                hidden_dim=config.get("head_hidden", 128)
    ...
    def forward(
        ...
        if self.predict_releases:
            releases = self.release_head(fused)
            return inflows, releases
